...
public fun String.decodeBase64(): ByteString = decodeBase64Internal()

public fun String.decodeBase64OrNull(): ByteString? = try {
    decodeBase64Internal()
} catch (e: IllegalArgumentException) {
    null
}

public fun ByteString.encodeBase64(
    url: Boolean = false,
    pads: Boolean = false,
...
private fun String.decodeBase64Internal(): ByteString {
    ...
    repeat(stripped.length) {
        ...
        if (modulus == 0) {
            buffer.writeByte((word shr 16).toByte())
            buffer.writeByte((word shr 8).toByte())
            buffer.writeByte(word.toByte())
        }
    }
    ...
    when (modulus) {
        ...
        2 -> {
            buffer.writeByte(((word shr 4).toByte()))
        }

        3 -> {
            buffer.writeByte((word shr 10).toByte())
            buffer.writeByte((word shr 2).toByte())
        }
