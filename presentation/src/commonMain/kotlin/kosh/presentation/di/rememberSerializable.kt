...
class SerializableSaver<T>(private val serializer: KSerializer<T>, cbor: Cbor = Cbor) :
    Saver<T, ByteArray> {
...
@Composable
inline fun <reified T : Any> serializableSaver(): SerializableSaver<T> {
    return SerializableSaver(serializer(), rememberRouteScope().cbor)
}
