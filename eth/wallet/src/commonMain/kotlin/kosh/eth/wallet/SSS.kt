package kosh.eth.wallet

import kotlin.experimental.xor
import kotlin.jvm.JvmInline

public object SSS {

    public fun split(
        secret: ByteArray,
        shares: Int,
        threshold: Int,
    ): List<ByteArray> {
        require(secret.isNotEmpty()) { "secret cannot be empty" }
        require(shares in 2..256) { "shares must be at least 2 and at most 255" }
        require(threshold in 2..256) { "threshold must be at least 2 and at most 255" }
        require(shares >= threshold) { "shares cannot be less than threshold" }

        val xCoordinates = ByteArray(shares).fillSecureRandom()

        val result = List(shares) {
            val share = ByteArray(secret.size + 1)
            share[secret.size] = (xCoordinates[it].toInt() + 1).toByte()
            share
        }

        for (i in secret.indices) {
            val byte = secret[i]
            val polynomial = makePolynomial(byte, threshold - 1)

            for (j in 0..<shares) {
                val x = (xCoordinates[j].toInt() + 1).toByte()
                val y = polynomial.evaluate(x)
                result[j][i] = y
            }
        }

        return result
    }

    public fun combine(shares: List<ByteArray>): ByteArray {
        require(shares.size in 2..256) { "shares must have at least 2 and at most 255 elements" }
        val share1 = shares[0]
        for (share in shares) {
            require(share.size >= 2) { "each share must be at least 2 bytes" }
            require(share.size == share1.size) { "all shares must have the same byte length" }
        }

        val secret = ByteArray(share1.size - 1)
        val xSamples = ByteArray(shares.size)
        val ySamples = ByteArray(shares.size)
        val checkSet = mutableSetOf<Byte>()

        for (i in shares.indices) {
            val share = shares[i]
            val sample = share.last()

            check(checkSet.add(sample)) { "shares must contain unique values but a duplicate was found" }
            xSamples[i] = sample
        }

        for (i in secret.indices) {
            for (j in shares.indices) {
                ySamples[j] = shares[j][i]
            }

            secret[i] = interpolatePolynomial(xSamples, ySamples, 0)
        }

        return secret
    }
}

private val Log_TABLE = shortArrayOf(
    ,
)

private val EXP_TABLE = shortArrayOf(
    ,
)

@JvmInline
internal value class Polynomial(val coefficients: ByteArray)

internal fun add(a: Byte, b: Byte): Byte {
    return a xor b
}

internal fun div(a: Byte, b: Byte): Byte {
    check(b != 0.toByte()) { "divide by zero" }

    val logA = Log_TABLE[a.toUByte().toInt()]
    val logB = Log_TABLE[b.toUByte().toInt()]
    val diff = (logA - logB + 255) % 255
    val result = EXP_TABLE[diff].toByte()

    return if (a == 0.toByte()) 0 else result
}

internal fun mult(a: Byte, b: Byte): Byte {
    val logA = Log_TABLE[a.toUByte().toInt()]
    val logB = Log_TABLE[b.toUByte().toInt()]
    val sum = (logA.toInt() + logB.toInt()) % 255
    val result = EXP_TABLE[sum]
    return if (a == 0.toByte() || b == 0.toByte()) 0.toByte() else result.toByte()
}

internal fun interpolatePolynomial(xSamples: ByteArray, ySamples: ByteArray, x: Byte): Byte {
    val limit = xSamples.size

    var basis: Byte
    var result: Byte = 0

    for (i in 0..<limit) {
        basis = 1

        for (j in 0..<limit) {
            if (i == j) {
                continue
            }
            val num = add(x, xSamples[j])
            val denom = add(xSamples[i], xSamples[j])
            val term = div(num, denom)
            basis = mult(basis, term)
        }

        val group = mult(ySamples[i], basis)
        result = add(result, group)
    }

    return result
}

internal fun Polynomial.evaluate(x: Byte): Byte {
    if (x == 0.toByte()) {
        return coefficients[0]
    }

    val degree = coefficients.size - 1
    var result = coefficients[degree]
    for (i in (degree - 1) downTo 0) {
        val coefficient = coefficients[i]
        result = add(mult(result, x), coefficient)
    }

    return result
}

internal fun makePolynomial(intercept: Byte, degree: Int): Polynomial {
    val polynomial = Polynomial(ByteArray(degree + 1))
    polynomial.coefficients.fillSecureRandom()
    polynomial.coefficients[0] = intercept
    return polynomial
}
